...
from model import parse_explore_rsp, parse_reflect_rsp, OpenAIModel, QwenModel, GeminiModel  # UPDATED IMPORT
from utils import print_with_color, draw_bbox_multi
# importing libraries over
setup_logger() # this is called to write output in the output.txt also


# basic definition for program like where is root directory and what is app name
...
# calling voice-assistant for story telling
...
if configs["MODEL"] == "OpenAI":
...
elif configs["MODEL"] == "Gemini":  # ADDED GEMINI SUPPORT
    ...
    sys.exit()
# starter code what is the root directory and all
app = args["app"]
...
if not app:
    ...
    app = input()
    app = app.replace(" ", "")
# work_directory for saving screeen_shots,logs etc.
...
# everything related to logs here is over

# which andriod app are currently running
...
# Real play begins here Agent greets you here
greeting_text = "Hi, I am Aagya, your app agent of L and T Finance. How may I help you?"
print_with_color(greeting_text,"Pink")
# Make the agent speak the greeting
voice_assistant.speak_text(greeting_text) # agent yaha pr hi hello krlega

task_desc = "" # now task_description is always to be read from input.txt
input_file_path = "input.txt" # Define the path to your input file
open(input_file_path, "w").close()
call_voice_assistant() # voice_assistant will write the task_description to input.txt
...
# task_description is ready now to be used
    if task_desc:
        print_with_color(f"Loaded task description from '{input_file_path}': '{task_desc}'", "green")
        # Clear the file so it's not used by the 'ask_human' function later by mistake.
        open(input_file_path, "w").close()
    else:
        print_with_color("ERROR: 'input.txt' was empty after proceeding. Exiting.", "red")
        sys.exit()
else:
    print_with_color(f"ERROR: '{input_file_path}' not found or is empty. Exiting.", "red")
    sys.exit()
# open(input_file_path, "w").close()
...
while round_count < configs["MAX_ROUNDS"]:
    ...
    # Check if there is a pending answer from the user
    if "human_input" in locals() and human_input:
        human_answer_context = f"You previously asked a question and the human responded with: '{human_input}'. Use this information for your next action."
        # Clear the variable so it's only used once
        human_input = ""
    ...
    if status:
        ...
        if act_name == "tap":
            ...
            print("swipe_dir and dist are ",swipe_dir,dist)
            tl, br = elem_list[area - 1].bbox
            x, y = (tl[0] + br[0]) // 2, (tl[1] + br[1]) // 2
            ...
            # NEW: Logic for handling a question to the user
            _, question_to_ask = res
            print_with_color(f"AGENT REQUEST: {question_to_ask}", "green")
            human_input = ""
            input_file_path = "input.txt"
            open(input_file_path, "w").close()
            print_with_color(f"Please write your answer in the 'input.txt' file.", "blue")
            call_voice_assistant()
            if os.path.exists(input_file_path) and os.path.getsize(input_file_path) > 0:
                with open(input_file_path, "r", encoding='utf-8') as f:
                    human_input = f.read().strip()
                print_with_color(f"Found response in '{input_file_path}': '{human_input}'", "cyan")
                # Clear the file so it's not used again in the next round by mistake
                open(input_file_path, "w").close()
            else:
                # 3. If file is not found or is empty, fall back to terminal input
                print_with_color("Waiting for your response. You can either type here or create/update 'input.txt' with your answer.", "yellow")
                human_input = input("Your terminal response: ")

            print_with_color("Understood. I will use this information for my next step.", "cyan")
            # Immediately start the next round with the new information
            continue
        elif act_name == "text":
            # MODIFIED: Logic for handling text input robustly
            _, area, input_str = res
            tl, br = elem_list[area - 1].bbox
            x, y = (tl[0] + br[0]) // 2, (tl[1] + br[1]) // 2

            # Tap first to ensure focus
            print_with_color(f"Tapping element {area} to focus before typing.", "cyan")
            controller.tap(x, y)
            time.sleep(1.5)  # Wait for keyboard to appear and focus to settle
            # Then type the text
    ...
    # print("response after taking ss ",rsp)
    if status:
        ...
        break
    print("time to have a summary of what has happened till now")
